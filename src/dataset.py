...
from config import DATA_FILES, DATA_URLS
...
def download_fake_news_data(force: bool = False) -> bool:
    ...
    for file_key, url_key, columns in tasks:
        ...
        if force or not file_path.exists():
            if not download_file(DATA_URLS[url_key], file_path) or not validate_dataset(
                file_path, columns
            ):
                success = False
        else:
...
def download(
    force: bool = typer.Option(
        False, "--force", "-f", help="Force re-download even if files exist"
    ),
):
